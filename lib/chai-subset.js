        ...
        function compare(expected, actual, path) {
            ...
            if (Array.isArray(expected)) {
                if (!Array.isArray(actual)) {
                    throw 'Expected an Array but got "' + typeof (actual) + '" at path "' + path + '".';
                }
                if (expected.length !== actual.length) {
                    throw 'Expected array of length "' + expected.length + '" but got "' + actual.length + '" at path "' + path + '".';
                }
                for (var i = 0; i < expected.length; i++) {
                    compare(expected[i], actual[i], path + (path === '/' ? '' : '/') + i);
                }
                return true;
            }
